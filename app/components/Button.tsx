...
export default function Button({
  ...
  if (variant === "link") {
    return (
      <button
        className={cn(
          "font-body text-xs font-medium uppercase tracking-[0.2em] text-text-main hover:text-accent transition-colors duration-500 cursor-pointer focus-visible:outline-none",
          className
        )}
        {...props}
      >
  ...
  return (
    ...
    >
      ...
      <span
        className="absolute inset-0 bg-text-main -translate-x-full group-hover:translate-x-0 transition-transform duration-500 ease-[cubic-bezier(0.25,0.46,0.45,0.94)]"
        aria-hidden="true"
      />
